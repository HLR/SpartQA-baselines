...
# def train(model, criterion, optimizer,pretrain, baseline, start, num_sample, train_num, qtype, human, other, device, file):
...
#     if pretrain == 'tokencls':
        
#         if train_num == 'train24k':
#             with open('dataset/SpRL_annotation_train_24k.json') as json_file:
#                 data = json.load(json_file)
                
#         elif train_num == 'train100k':
#             with open('dataset/SpRL_annotation_train_100k.json') as json_file:
#                 data = json.load(json_file)
                
#         elif train_num == 'train500':
#             with open('dataset/SpRL_annotation_train_500.json') as json_file:
#                 data = json.load(json_file)
#         else:
#             with open('dataset/SpRL_annotation_train.json') as json_file:
#                 data = json.load(json_file)
       
#         TPFN,TPFP,TP = np.array([0]*5),np.array([0]*5),np.array([0]*5)
...
#             #annotations of stories
#             for annot in story['story']['annotations']: 
                
# #                 if len(annot['spatial_description'])>1 or len(annot['spatial_description']) == 0: print(annot['sentence']);continue
                
#                 if len(annot['spatial_description']) == 0: print('Sentence:\n',annot['sentence'], file = file); print('Sentence:\n',annot['sentence']);continue
            
#                 sentence, traj, land, spatial = '', [], [], []
#                 traj_ind, land_ind, spatial_ind = [],[],[]
#                 sentence = annot['sentence']
#                 if sentence == '': print("problem in empty tag");continue
#                 print('Sentence:\n',sentence, file = file)
# #                 print('Sentence:\n',sentence)
                
#                 for spatial_item in annot['spatial_description']:
                    
#                     if [spatial_item['trajector']['phrase'] == '' or spatial_item['landmark']['phrase']] == '' or spatial_item['spatial_indicator']['phrase'] == '': print("problem in empty tag");continue
                        
#                     if spatial_item['trajector']['SOT_sentence'] not in traj_ind:     
# #                         traj += [spatial_item['trajector']['phrase']]
#                         traj_ind += [spatial_item['trajector']['SOT_sentence']]
                    
#                     if spatial_item['landmark']['SOT_sentence'] not in land_ind:    
# #                         land += [spatial_item['landmark']['phrase']]
#                         land_ind += [spatial_item['landmark']['SOT_sentence']]
                    
#                     if spatial_item['spatial_indicator']['SOT_sentence'] not in spatial_ind:
# #                         spatial += [spatial_item['spatial_indicator']['phrase']]
#                         spatial_ind += [spatial_item['spatial_indicator']['SOT_sentence']]
                
                
#                 print(traj_ind, land_ind, spatial_ind, file = file)    
# #                 label = make_label_token(tokenizing(sentence), sentence, traj_ind, land_ind, spatial_ind)
                
#                 model.zero_grad()
#                 all_q += 1
                
                
#                 loss, output, truth = token_classification(model, sentence, traj_ind, land_ind, spatial_ind , other, device, file)
                
#                 print("truth: ", truth, "\npredict: ", output, file = file)
#                 correct, TP,TPFP,TPFN = confusion_matrix(truth, output,correct, TP,TPFP,TPFN)
...
#             #annotations of questions
#             for annot in story['questions']: 
                
# #                 for annot in question['annotations'][0]['spatial_description']:
# #                 if len(annot['spatial_description'])>1 or len(annot['spatial_description']) == 0: print(annot['sentence']);continue
                
#                 if len(annot['annotation'][0]['spatial_description']) == 0: print('Question:\n',annot['question'], file = file); print('Question:\n',annot['question']);continue
            
#                 sentence, traj, land, spatial = '', [], [], []
#                 traj_ind, land_ind, spatial_ind = [],[],[]
#                 sentence = annot['question']
#                 if sentence == '': print("problem in empty tag");continue
#                 print('Question:\n',sentence, file = file)
# #                 print('\nQuestion:',sentence, "\n")
# #                 print('Sentence:\n',sentence)
                
#                 for spatial_item in annot['annotation'][0]['spatial_description']:
...
# #                 print('rels: ',traj_ind, land_ind, spatial_ind)
#                 print(traj_ind, land_ind, spatial_ind, file = file)    
# #                 label = make_label_token(tokenizing(sentence), sentence, traj_ind, land_ind, spatial_ind)
...
#         print('Train Losses: ', losses, file = file)
        
#         print('TP:',TP, '  TPFP: ', TPFP,'   TPFN: ', TPFN ,file = file)
#         #precision
#         p = precision(TP,TPFP)
# #         print('Train Precision: ', p)
#         print('Train Precision: ', p, file=file)
        
#         #recall
#         r = recall(TP,TPFN)
# #         print('Train Recall: ', r)
#         print('Train Recall: ', r, file=file)
        
#         #f1
#         f= F1_measure(TP,TPFP, TPFN)
# #         print('Train F1: ', f)
#         print('Train F1: ', f, file=file)
        
#         #macro-f1
#         mf = F1_measure(TP,TPFP, TPFN, macro = True)
#         print('Train Macro-F1: ', mf)
#         print('Train Macro-F1: ', mf, file=file)
        
#         #accuracy
#         print('Train accuracy: ', correct / all_q, file=file)
...
#     return losses, correct/ all_q
# model, criterion, optimizer,pretrain, baseline, start, num_sample, train_num, qtype, human, other, device, file
def train(model, criterion, optimizer,pretrain, baseline, start, num_sample, train24k, qtype, human, other, device, file):
    
    #import baseline
    if baseline == 'bert':
        from BERT import tokenizing,  boolean_classification
#     elif baseline == 'xlnet':
#         from XLNet import tokenizing, multiple_choice, boolean_classification
#     elif baseline == 'albert':
#         from ALBERT import tokenizing, multiple_choice, boolean_classification
